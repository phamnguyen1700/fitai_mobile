...
class AuthRepository {
  ...
  Future<AuthResponse> login({
    ...
    try {
      ...
      // Case sai tài khoản / mật khẩu: backend trả success = false + message
      if (response.success && response.token != null) {
        await _apiClient.storeTokens(
          accessToken: response.token!,
          userData: response.userData != null
              ? jsonEncode(response.userData!.toJson())
              : null,
        );
      }
  ...
  Future<UserModel?> getCurrentUser() async {
    ...
    try {
      ...
      if (userData != null) {
        ...
        cachedUser = UserModel.fromJson(userJson);
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.success && response.userData != null) {
        await updateUserData(response.userData!);
  ...
  Future<AuthResponse> verifyEmail({
    ...
    try {
      return await _authApiService.verifyEmail(userId: userId, hash: hash);
    } catch (e) {
  ...
  Future<AuthResponse> verifyOtp({
    ...
    try {
      ...
      // If API returns user data in data field, optionally persist
      if (response.success && response.userData != null) {
        await updateUserData(response.userData!);
      }
